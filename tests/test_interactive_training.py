...
def test_show_message_wraps_long_text() -> None:
    class FakeFont:
        def size(self, text):
    ...
    fake_display.WHITE = (255, 255, 255)
    fake_display.BLACK = (0, 0, 0)
    fake_display.font = FakeFont()
    fake_display.small_font = FakeFont()
    fake_display.screen = FakeScreen()
    fake_display._clamp = interactive_training.InspectionDisplay._clamp
    fake_display._reflow_layout = lambda: None
    fake_display._wrap_text_lines = lambda text, max_width, font=None: interactive_training.InspectionDisplay._wrap_text_lines(fake_display, text, max_width, font=font)

    original_flip = interactive_training.pygame.display.flip
    interactive_training.pygame.display.flip = lambda: None
    try:
        interactive_training.InspectionDisplay.show_message(
            fake_display,
            "This startup warning should wrap into multiple readable lines instead of clipping off the screen.",
        )
    finally:
        interactive_training.pygame.display.flip = original_flip
...
def test_run_interactive_training_waits_for_operator_before_first_capture(monkeypatch, tmp_path) -> None:
    config_path = tmp_path / "camera_config.json"
    config_path.write_text(json.dumps({"inspection": {}, "training": {}}, indent=2) + "\n", encoding="utf-8")

    active_paths = {
        "config_file": config_path,
        "reference_dir": tmp_path / "reference",
        "reference_mask": tmp_path / "reference" / "golden_reference_mask.png",
        "reference_image": tmp_path / "reference" / "golden_reference_image.png",
        "log_dir": tmp_path / "logs",
    }
    ...
    active_paths["log_dir"].mkdir(parents=True, exist_ok=True)
    ...
    class FakeDisplay:
        ...
        def __init__(self, *args, **kwargs):
            self.ready_prompts = 0

        def wait_for_capture_start(self, config, has_reference, mode_lines=None):
            self.ready_prompts += 1
            return "home"

        def set_alignment_profile_label(self, profile):
            return None
    ...
    class FakeLogger:
        ...
        def log_review_findings(self, warnings):
            return None
    ...
    class FakeTrainer:
        def __init__(self, *args, **kwargs):
            self.active_paths = kwargs.get("active_paths")

        def get_training_review_warnings(self, *args, **kwargs):
            return []
    ...
    monkeypatch.setattr(interactive_training, "PYGAME_AVAILABLE", True)
    monkeypatch.setattr(interactive_training, "InspectionDisplay", FakeDisplay)
    monkeypatch.setattr(interactive_training, "TrainingLogger", FakeLogger)
    monkeypatch.setattr(interactive_training, "ThresholdTrainer", FakeTrainer)
    monkeypatch.setattr(interactive_training, "IndicatorLED", FakeIndicator)
    monkeypatch.setattr(interactive_training, "get_active_runtime_paths", lambda: active_paths)
    monkeypatch.setattr(
        interactive_training,
    ...
    monkeypatch.setattr(interactive_training, "get_inspection_runtime_warnings", lambda *args, **kwargs: [])
    monkeypatch.setattr(interactive_training, "format_operator_mode_lines", lambda *args, **kwargs: ["Mode: training"])
    monkeypatch.setattr(interactive_training, "capture_to_temp", lambda config: (_ for _ in ()).throw(AssertionError("capture_to_temp should not run before operator starts capture")))

    result = interactive_training.run_interactive_training({"inspection": {}, "training": {}})
